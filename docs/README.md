...
- [`CLI Guide`](./start/cli-guide.md) — `factory` / `<repo-name>` 명령어 전체 매트릭스
...
- [`Dogfood Setup`](./start/dogfood-setup.md)
  ...
  - [`Dogfood Walkthrough`](./start/dogfood-walkthrough.md) — 사이클 흐름과 정착된 패턴
...
- [`개요`](./philosophy/README.md) — 프롤로그 (3 제약) + 39 ADR 인덱스
...
- [`Core Schema 폐기`](./philosophy/adr-037-core-schema-deprecation.md) — ADR-037
- [`SMS + 휴대폰 점유인증`](./philosophy/adr-038-sms-phone-auth.md) — ADR-038
- [`Admin 모듈 (cross-app 운영 콘솔)`](./philosophy/adr-039-admin-module.md) — ADR-039
...
- [`Dynamic Query`](./convention/dynamic-query.md) — 동적 쿼리 표준 패턴
- [`Code Comments`](./convention/code-comments.md)
...
- [`Swagger UI`](./api-and-functional/api/swagger-ui.md) — API 탐색
...
- [`Billing / IAP / Payment`](./api-and-functional/functional/billing-iap-payment.md) — 결제 도메인 통합 가이드
- [`Phone Auth & SMS`](./api-and-functional/functional/phone-auth-and-sms.md) — 점유인증 + SMS 발송
...
- [`Infra Decisions`](./production/deploy/decisions-infra.md) — I-01~I-14
...
- [`Flyway Runbook`](./production/deploy/flyway-runbook.md) — 환경별 모드 · migrate-prod.sh

**Operations**
- [`Develop Branch Policy`](./production/operations/develop-branch-policy.md)
- [`Feature Toggle`](./production/operations/feature-toggle.md) — Lite 모드 운영

**Setup**
- [`Key Issuance`](./production/setup/key-issuance.md) — 운영 키 발급 통합 가이드
- [`Key Rotation`](./production/setup/key-rotation.md)
- [`Secret Chain 4-Stage`](./production/setup/secret-chain-4stage.md) — secret 4곳 동기화
- [`Mac Mini Setup`](./production/setup/mac-mini-setup.md)
- [`Monitoring Setup`](./production/setup/monitoring-setup.md)
- [`Storage Bucket Isolation`](./production/setup/storage-bucket-isolation.md)
- [`Storage Setup`](./production/setup/storage-setup.md)
- [`OWASP Top 10 Mapping`](./production/setup/owasp-top10-mapping.md)
...
- [`코어 데이터 모델`](./reference/data-model.md) — 코어 21 테이블 + admin 스키마 (컬럼 · FK · 인덱스 · 마이그레이션 V번호)
- [`API Reference`](./reference/api/README.md) — 엔드포인트 카탈로그
- [`용어 사전`](./reference/glossary.md) — Spring · JPA · Docker · JWT · Kamal 등
- [`Environment`](./reference/environment.md) — 프레임워크 · 라이브러리 · 외부 서비스 인벤토리
- [`Environment UI`](./reference/environment-ui.md) — 기술 스택 다이어그램
