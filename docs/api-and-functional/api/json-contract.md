...
이 문서는 DTO JSON 직렬화/역직렬화의 전역 정책과 테스트 규약을 정의해요.

---

## 개요

DTO JSON **직렬화/역직렬화 정책** + 필수 테스트 4 종 (샘플/보안/중첩/타입) 목록입니다. 필드 추가/변경 시 절차도 포함해요.

---

## 전역 Jackson 정책

모든 DTO 는 다음 정책을 따릅니다. 정책은 `AbstractJsonContractTest` 내부의 `contractObjectMapper()` 에 내장되어 있고, 프로덕션 Spring Boot 기본 Jackson 설정과 일치해야 합니다.
...
Response DTO 에는 validation 어노테이션이 불필요해요 (서버가 생성하니까요).

### 접미사 규약 ([`naming.md`](../../convention/naming.md) 참조)
...
모든 DTO 는 `AbstractJsonContractTest<T>` 상속으로 자동 수행합니다.
...
특수 케이스가 필요하면 개별 `@Test` 메서드를 추가해요.
...
`@JsonTest` 어노테이션은 **불필요해요**. Spring 컨텍스트 없이 순수 `ObjectMapper` 기반으로 동작합니다.

---

## 샘플 — 보안 민감 필드

`passwordHash` 같은 필드는 JSON 에 존재하지만 값은 노출하면 안 됩니다. 테스트에서 **존재만 확인** 해요.
...
`AuthResponse` 는 `UserSummary` 와 `AuthTokens` 를 포함합니다.
...
중첩 record 도 `equals` 가 자동으로 생성되니까 round-trip 테스트가 그대로 작동해요.
...
`JavaTimeModule` 이 자동 등록되어 있어서 `@JsonFormat` 이 불필요합니다.
...
**Map 순서** — Jackson 기본은 insertion order 입니다. `HashMap` 대신 `LinkedHashMap` 을 사용해서 canonicalJson 과 일치시켜요.
...
4. 테스트 실행 — 실패 시 위 둘을 조정

### 필드 타입 변경 (breaking)

- 이런 변경은 **버저닝의 breaking change** 에 해당해요.
- `@Deprecated` 로 기존 필드를 유지하고 새 필드를 추가합니다 → 한 번에 교체는 금지예요.
- 자세한 절차는 [`versioning.md`](./versioning.md) 의 Deprecation 프로세스를 참조하세요.

### 필드 제거

- 소비자 앱 버전 호환성을 위해 `@Deprecated` 기간을 거친 후 제거합니다.
- 제거 시점에 Json 테스트도 동시에 수정해요.
...
- [ ] 접미사 규약 준수 ([`naming.md`](../../convention/naming.md))
...
- [`API Response Format`](./api-response.md) — `ApiResponse<T>` 표준 envelope
- [`Versioning`](./versioning.md) — breaking change 절차
