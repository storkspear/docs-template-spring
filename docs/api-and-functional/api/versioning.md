...
앱 도메인 경로(auth · user · device · payment · iap · notification-settings · posts)와 운영 콘솔(admin), 그리고 `SecurityConfig` 패턴은 전부 `common-web` 의 [`ApiEndpoints`](../../../common/common-web/src/main/java/com/factory/common/web/ApiEndpoints.java) 상수 한 곳에서 관리돼요 — 이 컨트롤러들은 core 의 공유 런타임 빈이라서요 ([`ADR-013`](../../philosophy/adr-013-per-app-auth-endpoints.md) B). 각 앱의 `<Slug>ApiEndpoints` 는 `BASE`(`/api/apps/<slug>`)와 헬스체크 경로만 들고 있어요. 경로 prefix 는 두 가지뿐이에요.
...
> 2026-07-21 버전 리셋: 운영 릴리스가 없던 0.x 태그를 정리하고 template-v1.0.0 부터 아래 주기를 적용해요.
