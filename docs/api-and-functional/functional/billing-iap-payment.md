...
운영 환경에서 자주 부딪히는 함정 중 하나입니다. `<your-backend> new <slug>` 가 생성하는 슬러그 컨트롤러(`*PaymentController`) 는 `PaymentPort` 를 *필수 의존* 으로 가집니다. 이 때문에 prod 와 dev 프로파일에서 `PortOneProdConfigGuard` 가 부팅 시점에 PortOne 설정을 검증해요.
...
답은 "코드는 이미 완성·테스트되어 있어 키만 채우면 동작" 입니다. `core-payment` 에는 검증·환불·재결제를 담당하는 `PortOneAdapter`, HMAC SHA-256 과 timestamp 를 검증하는 `PortOneWebhookVerifier`, 슬러그 `…PaymentController`, 부팅 가드(`PaymentAutoConfiguration`), WireMock stub, 단위 테스트가 모두 들어있어요. 채널 추가 코드 작업은 없습니다.
...
빈이 없어도 `BillingServiceImpl` 은 `ObjectProvider` 로 lazy 의존하므로 부팅 자체는 통과해요. 대신 결제 경로를 *실제로 호출* 하는 시점에 `CommonError.FEATURE_DISABLED`(CMN_009) 를 던집니다. 슬러그 컨트롤러가 `PaymentPort` 를 직접 주입받는 구조라면, 빈이 없을 때 컨트롤러 등록 단계에서 부팅이 실패할 수 있으니 토글을 끌 때는 컨트롤러 의존 형태를 함께 확인하세요.

따라서 결제 도메인을 완전히 쓰지 않으려면 두 가지 중 하나를 선택합니다. 첫째는 슬러그 컨트롤러를 삭제하는 방법이고, 둘째는 §4 의 fallback(자격 셋 다 비움) 으로 두는 방법이에요. 도그푸딩 단계에서는 후자가 단순하고 안전합니다. 토글의 운영 규칙 전체는 [`Feature Toggle`](../../production/operations/feature-toggle.md) 을 참고하세요.
