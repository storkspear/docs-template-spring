...
`RateLimitFilterTest` 가 필터 동작을 7개 단위 테스트로 검증해요. strict 한도까지의 정상 통과, 초과 시 429, 앱별·IP별 버킷 분리, 비민감 경로의 default 적용, `X-Forwarded-For` 우선순위, 전역 비활성화 동작을 각각 확인합니다. 초과 케이스는 strict 한도를 3으로 잡고 4번째 호출에서 429 와 `CMN_429` 가 나오는지를 봅니다.
