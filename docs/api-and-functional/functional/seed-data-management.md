...
`new-app.sh` 가 만드는 `db/migration/<slug>/` 의 versioned 파일은 다음과 같습니다. V001~V017 이 공통 테이블이고, V007 자리만 admin 시드(DML)이며, 도메인 테이블은 **V018 부터** 이어집니다.
...
| `V018__...` 이후 | 파생 레포의 도메인 테이블 (직접 작성) |
...
├── V018__init_expense_categories.sql    ← 도메인 스키마
...
새 앱을 `./tools/new-app/new-app.sh <slug>` 로 생성하면 `db/migration/<slug>/` 에 V001~V017 (공통 테이블 + V007 admin 시드) 가 자동으로 만들어집니다. 각 앱은 자기 schema 를 가지며, Flyway 는 `classpath:db/migration/<slug>` 를 해당 schema 에 대해 독립적으로 실행해요 (`common-persistence/src/main/java/com/factory/common/persistence/AbstractAppDataSourceConfig.java` 의 `buildFlyway` 참조).

### 도메인 테이블과 seed 추가

앱 고유 도메인 테이블은 V018 부터 이어서 추가합니다. 참조 데이터는 옵션 A 로 같은 디렉터리에 둬요.

```
apps/app-sumtally/src/main/resources/db/migration/sumtally/
├── V001 ~ V006                              ← 유저·인증 (자동 생성)
├── V007__seed_admin_user.sql                ← admin 시드 (자동 생성)
├── V008 ~ V017                              ← 결제·audit·2FA·알림·점유인증·활동추적 (자동 생성)
├── V018__init_expense_categories.sql        ← 도메인 스키마
├── V019__init_expenses.sql                  ← 도메인 스키마
