...
모듈이 작으면 하위 패키지를 생략할 수 있습니다. `common-logging` 처럼 클래스가 3~4개뿐이면 단일 패키지에 모아도 됩니다.
...
@Service
class EmailAuthService { ... }          // core-auth-impl 내부 조립 용도
```

### 컨트롤러

REST 컨트롤러는 `XxxController` 를 씁니다. 인증과 유저 컨트롤러는 각 앱 모듈에 위치해요.
...
| 일반 | `Dto` | 위 분류에 안 맞는 교환 객체 | `DeviceDto`, `PlanDto` |
...
`new app` 스크립트가 V001~V017 공통 마이그레이션을 자동으로 깔아 주고, 도메인 테이블은 개발자가 V018부터 작성합니다. V007은 도메인이 아니라 admin 시드라서 그다음 빈 번호가 V018이에요.
...
apps/app-sumtally/src/main/resources/db/migration/sumtally/
    ...
    ...
    V017__init_user_activity_days.sql       ← new-app.sh 자동 생성 (마지막 공통, DAU/MAU 추적)
    V018__init_budget_groups.sql            ← 개발자가 작성 (첫 도메인 테이블)
    V019__init_expenses.sql                 ← 개발자가 작성
```

공통 마이그레이션 V001~V017 의 내용 분류는 [`onboarding.md`](../start/onboarding.md#3-첫-앱-모듈-추가) 의 표에 정리돼 있어요.

각 앱 schema 는 자기 디렉토리에서 유저/인증 테이블(V001~V017)과 도메인 테이블(V018~)을 함께 관리합니다. Flyway 는 각 디렉토리를 자기 schema 에 대해 독립적으로 적용해요.
...
[`ADR-010`](../philosophy/adr-010-search-condition.md) 이 정의하는 표준 연산자는 8개예요. `eq`(기본), `gte`, `lte`, `gt`, `lt`, `like`, `isNull`, `isNotNull`. 상세 규칙과 `common-web` ↔ `common-persistence` 분리 이유는 [`ADR-010 (SearchCondition)`](../philosophy/adr-010-search-condition.md) 을 참고하세요.
...
POST   /api/apps/{slug}/auth/password-reset          # 비밀번호 재설정 요청
...
GET    /api/apps/{slug}/users/me/devices     # 내 디바이스 목록
