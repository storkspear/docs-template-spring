...
`new app` 스크립트가 V001~V015 공통 마이그레이션을 자동으로 깔아 주고, 도메인 테이블은 개발자가 V016부터 작성합니다. V007은 도메인이 아니라 admin 시드라서 그다음 빈 번호가 V016이에요.
...
apps/app-sumtally/src/main/resources/db/migration/sumtally/
    ...
    ...
    V015__init_auth_phone_verification_codes.sql ← new-app.sh 자동 생성 (마지막 공통)
    V016__init_budget_groups.sql            ← 개발자가 작성 (첫 도메인 테이블)
    V017__init_expenses.sql                 ← 개발자가 작성
```

공통 마이그레이션 V001~V015 의 내용 분류는 [`onboarding.md`](../start/onboarding.md#3-첫-앱-모듈-추가) 의 표에 정리돼 있어요.

각 앱 schema 는 자기 디렉토리에서 유저/인증 테이블(V001~V015)과 도메인 테이블(V016~)을 함께 관리합니다. Flyway 는 각 디렉토리를 자기 schema 에 대해 독립적으로 적용해요.
