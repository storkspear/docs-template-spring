...
첫째, 운영자가 `tools/migrate-prod.sh` 로 배포 전에 직접 적용하는 방법이에요. 이 스크립트가 마이그레이션 SQL 을 트랜잭션 안에서 실행하고 Flyway 이력에 기록까지 남겨요. 둘째, 이미지의 `migrate-only` 모드를 쓰는 방법이에요. Kamal 이 Green 컨테이너를 띄우기 전에 이 모드로 한 번 실행해, DB 마이그레이션만 끝내고 빠져나와요 (web 서버는 띄우지 않아요). [`Blue/Green 배포`](../reference/glossary.md#운영--인프라) 가 겹치는 구간에서 Flyway 가 동시에 도는 레이스를 피하려는 설계예요.
...
| 알림 | Discord | [`Alertmanager`](../reference/glossary.md#관측성--로깅) 가 CPU · 메모리 · 5xx · p95 임계 초과 시 발송 |
