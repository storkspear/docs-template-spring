...
**모든 결정이 이 세 제약의 세 가지 조합에서 나옵니다.** 프롤로그를 먼저 읽어두면 이후 ADR 독해가 훨씬 수월해질 거예요.

---

## 이 문서의 사용법

이 문서는 **39개의 ADR 카드** 로 구성되어 있으며, 각 카드는 하나의 설계 결정을 다룹니다. 전체를 순서대로 읽는 것이 가장 좋지만, 독자의 상황과 목적에 따라 진입점이 달라질 수 있습니다.
...
| "솔로 운영자가 여러 앱을 한 화면에서 보려면? (cross-app 콘솔)" | [`ADR-039: admin 모듈 (superadmin + admin 스키마 + in-process fan-out)`](./adr-039-admin-module.md) |

> **39 개 ADR 작성 완료** (ADR-001 ~ ADR-039). 테마별로 그룹화되어 있고 각 카드는 독립적으로 읽을 수 있습니다.
...
**테마 1 의 결론**: 하나의 JAR 안에 여러 앱이 공존하되, 모듈 경계는 기계가 강제하고, 도메인은 파생 레포에 두며, 미래의 추출 가능성을 위한 포트 인터페이스를 함께 유지합니다.
...
ADR-034 (Feature Toggle Lite mode)
  "@ConditionalOnProperty 기반 opt-out — 11 도메인 토글 가능"
   │
   │ 사용자 UI 에는 어떻게 노출?
   ▼
...
- [`ADR-039 · admin 모듈 (cross-app 운영 콘솔 — superadmin + admin 스키마 + in-process fan-out)`](./adr-039-admin-module.md)

**테마 8 의 결론**: 운영 안전성과 유연성을 동시에 확보하는 운영 정책 + 외부 호출 보안 baseline. Flyway 는 dev/test 에서는 자동 migrate, prod 는 validate 만 하고 운영자가 명시적으로 적용 (ADR-033 — `tools/migrate-prod.sh`). Lite 모드는 11 개 도메인 (payment / iap / email / sms / 2fa / audit / push / billing-notification / password-policy / phone-auth / analytics) 을 `.env.prod` 의 환경변수로 opt-out 가능 (ADR-034). 사용자 UI 차원의 노출은 ADR-035 에서 별도 정리. 외부 HTTP 호출 (Apple / Google / Kakao / Naver / Resend / MinIO / FCM) 은 모두 hardcode 또는 운영자 통제 환경변수로만 결정해 SSRF 가 설계상 불가능 (ADR-036). *솔로 인디 운영자가 자기 앱의 복잡도에 맞게 도메인을 골라 켜되, 외부 호출 보안 baseline 은 일관 유지하는* 흐름. ADR-037 이 지운 공유 core schema 위에, ADR-039 는 앱 데이터를 담지 않는 `admin` schema 하나만 예외로 얹어 솔로 운영자의 cross-app 가시성을 확보합니다.
...
| [`운영 콘솔 API`](../api-and-functional/admin-console.md) | superadmin 인증 + cross-app fan-out 엔드포인트 | ADR-039, 005, 012, 018, 037 |

---

## 템플릿 유지 규칙 (절대 금지)

이 템플릿 레포에 커밋할 때 **반드시** 지킵니다. 파생 레포에는 적용되지 않아요 — 거기에선 오히려 도메인 로직을 적극적으로 씁니다.

- **특정 앱/도메인/팀/회사 이름** 을 코드나 문서에 박지 않습니다. 템플릿이 중립적이어야 어느 도메인으로든 가지를 뻗을 수 있어요.
- **특정 인프라 자격증명, 계정 식별자, 프로젝트 ID** 를 커밋하지 않습니다. Supabase project-ref, Google Client ID, Firebase 키 등은 파생 레포의 `.env` 에서만 존재해야 합니다.
...
- **`docs/planned/archive/` 는 template-spring 전용 history** — 파생 레포 (server-factory 등) 와 docs 뷰어 레포 (docs-template-spring) 에는 미반영. `tools/sync-docs.sh` 가 자동 exclude 합니다.
