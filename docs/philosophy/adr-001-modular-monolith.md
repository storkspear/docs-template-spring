...
flowchart TB
    ...
    subgraph coreImpl["core/ -impl (비즈니스 구현 — 12 도메인)"]
        authImpl[core-auth-impl]
        userImpl[core-user-impl]
        pushImpl[core-push-impl]
        otherImpl[device · email · audit · billing · iap · payment · storage · sms · phone-auth]
    end

    subgraph coreApi["core/ -api (포트 인터페이스 — 12 도메인)"]
        authApi[core-auth-api]
        userApi[core-user-api]
        pushApi[core-push-api]
        otherApi[device · email · audit · billing · iap · payment · storage · sms · phone-auth]
    end
...
**모듈 경계가 기계에 의해 강제** — 5개 convention plugin + 22개 ArchUnit 규칙이 경계를 사람 의지로부터 분리합니다. 솔로 개발자가 피곤한 날에도 실수로 얽힘이 들어가지 않습니다.
...
**모듈 레이아웃**:
- [`settings.gradle`](https://github.com/storkspear/template-spring/blob/main/settings.gradle) — 전체 모듈 include (common × 5, core × 24 = api/impl 12 쌍, bootstrap × 1, apps × 0)
