...
**Status**: Accepted. ArchUnit 규칙 r1~r22 (r12 는 예약 공번 — 활성 21개) + 5개 Gradle convention plugin 으로 모듈 경계를 강제하고 있어요.
...
- **장점**: 가장 단순. 초기 셋업 거의 0.
- **단점**: Java 는 **폴더 구조와 의존 관계가 무관** 합니다. `apps/app-b/` 안의 클래스에서 `import com.factory.apps.app_a.SomeUtility` 가 그냥 컴파일됩니다. 한 번이라도 실수로 얽히기 시작하면 분리가 사실상 불가능해져요. 경계가 오로지 사람 의지에만 기대는 구조.
...
flowchart TB
    ...
    subgraph coreImpl["core/ -impl (비즈니스 구현 — 16 도메인)"]
        authImpl[core-auth-impl]
        userImpl[core-user-impl]
        pushImpl[core-push-impl]
        otherImpl[device · email · audit · billing · iap · payment · storage · sms · phone-auth · attachment · content · analytics · admin]
    end

    subgraph coreApi["core/ -api (포트 인터페이스 — 15 도메인, admin 은 -impl 전용)"]
        authApi[core-auth-api]
        userApi[core-user-api]
        pushApi[core-push-api]
        otherApi[device · email · audit · billing · iap · payment · storage · sms · phone-auth · attachment · content · analytics]
    end
...
canonical 규칙은 [`ArchitectureRules.java`](https://github.com/storkspear/template-spring/blob/main/common/common-testing/src/main/java/com/factory/common/testing/architecture/ArchitectureRules.java) 에 r1~r22 의 public static final ArchRule 상수로 정의돼 있습니다 (r12 는 예약 공번 — 활성 21개). 규칙 1~5 가 이 결정과 직접 연관됩니다.
...
**모듈 경계가 기계에 의해 강제** — 5개 convention plugin + 21개 ArchUnit 활성 규칙이 경계를 사람 의지로부터 분리합니다. 솔로 개발자가 피곤한 날에도 실수로 얽힘이 들어가지 않습니다.
...
> **갱신 (ADR-013 B, 2026-06)**: 아래 교훈의 결론(「`AuthController` 는 런타임 bean 이 아니라 `new-app.sh` 스캐폴딩 소스」)은 이후 [`ADR-013`](./adr-013-per-app-auth-endpoints.md) 의 `## 갱신` 에서 다시 뒤집혔어요. `AuthController` 는 현재 `AuthAutoConfiguration` 이 등록하는 **단일 공유 런타임 빈**이고, `/api/apps/{appSlug}/auth/*` 의 path 변수로 모든 앱을 처리합니다 — 당시 우려했던 ThreadLocal 통합 계정 라우팅 없이 appSlug 가 URL 에 명시되는 구조라, 아래 교훈의 문제의식 자체는 여전히 유효해요. 아래는 **역사적 기록**으로 남깁니다.
...
**모듈 레이아웃**:
- [`settings.gradle`](https://github.com/storkspear/template-spring/blob/main/settings.gradle) — 전체 모듈 include (common × 5, core × 31 = api/impl 15 쌍 + core-admin-impl, bootstrap × 1, apps × 0)
