...
> **이 ADR 의 범위** — [`ADR-001`](./adr-001-modular-monolith.md) 에서 "왜 2단계 방어가 필요한가" 를, [`ADR-003`](./adr-003-api-impl-split.md) 에서 "무엇을 분리하는가" 를 다뤘어요. -api 와 -impl 의 분리가 그 대상이에요. 본 ADR 은 "어떤 도구로 어떻게 강제할 것인가" 의 도구 선택과 DSL 설계, 그리고 앞에서 안 다룬 나머지 ArchUnit 규칙 r16, r18~r22 에 초점을 맞춥니다.
...
**설계 의도**: 단순 allowlist 보다 유연해요. 정규식을 지원하니까요. 그러면서도 복잡한 규칙 엔진보다 간단합니다. `forbiddenPattern` 을 우선 적용해서 "금지는 절대 금지" 원칙을 지켜요.
...
DSL 은 외부 Maven 라이브러리 의존을 **검증하지 않습니다**. 외부 라이브러리 경계 관리는 `dependency-check` 나 Snyk 같은 다른 도구의 몫이에요. 모듈 경계는 "내 프로젝트 안의 project 간 관계" 만 다룹니다.
...
**목적**: DTO 이름이 제각각이면 (`UserData`, `AuthInfo`, `TokenBundle` 등) 용도 파악이 어려워집니다. 접미사를 제한해서 이름만 보면 용도가 즉시 이해되게 했어요. **이름이 곧 문서** 가 되는 규칙이에요.
...
**규칙 canonical 위치 단일화** — `DependencyRules.groovy` 와 `ArchitectureRules.java` 두 파일만 보면 모든 아키텍처 규칙을 확인할 수 있어요.

**규칙이 데이터화됨** — `ArchitectureRules.java` 의 22개 `public static final ArchRule` 은 다른 테스트에서 재사용할 수 있어요. 규칙이 "lint 설정" 이 아니라 Java 오브젝트로 존재하니까요.
...
**교훈**: 기계 강제의 엄격함은 실무 필요에 의해 미세 조정되어야 합니다.

### 또 하나의 교훈 — 규칙을 "번호로" 식별하는 것의 가치

ArchUnit 규칙은 `r1`, `r2`, ..., `r22` 처럼 **번호로** 참조합니다.

- 빌드 실패 시: "`r11 violation: AuthPort.signIn() returns User`" 처럼 번호가 에러 메시지에 등장해요 → 번호로 검색해서 어떤 규칙인지 즉시 파악.
- ADR 작성 시: "r9 는 extraction-critical 이고..." 처럼 번호가 구분자 역할을 해요.
...
- **[Checkstyle](https://checkstyle.sourceforge.io/)** — Java 전통 lint.
- **[PMD](https://pmd.github.io/)** — Java 전통 lint (소스 분석).
- **[SpotBugs](https://spotbugs.github.io/)** — Java 전통 lint (바이트코드 분석).
