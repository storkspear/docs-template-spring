...
**Status**: Accepted. *Updated by [ADR-037](./adr-037-core-schema-deprecation.md)* — 공유 *core schema* 와 `coreDataSource` Bean 제거. 방어선 구성은 본문 그대로 **5중** (DB role · DataSource · Flyway · 포트 · ArchUnit) 입니다. `infra/scripts/init-app-schema.sql` + `common-persistence/AbstractAppDataSourceConfig.java` 로 구현되어 있어요. Supabase production 도 동일한 구조를 따릅니다.
...
3. `V001__init_users.sql` ~ `V025__add_analytics.sql` 템플릿 생성 (기본 24개 — `V007` 시드는 `--seed-admin` opt-in) → **방어선 3** (Flyway 경로)
...
**앱 추가 비용이 거의 제로** — `./tools/new-app/new-app.sh sumtally` 한 줄로 schema · role · DataSource · 기본 마이그레이션 24개 + AutoConfiguration 까지 셋업. 솔로 운영자가 새 앱 착수 시 DB 셋업 시간 5분 미만이에요.
...
- [`infra/scripts/init-app-schema.sql`](https://github.com/storkspear/template-spring/blob/main/infra/scripts/init-app-schema.sql) — 앱별 schema + role 생성, public schema 접근 차단 (`core` schema 초기화 스크립트는 ADR-037 로 폐기)
...
- [`RoutingDataSourceConfig.java`](https://github.com/storkspear/template-spring/blob/main/bootstrap/src/main/java/com/factory/bootstrap/config/RoutingDataSourceConfig.java) — slug 별 DataSource routing ([`ADR-037`](./adr-037-core-schema-deprecation.md) 로 `coreDataSource` Bean 대체)
...
- [`tools/new-app/new-app.sh`](https://github.com/storkspear/template-spring/blob/main/tools/new-app/new-app.sh) — 마이그레이션 템플릿 자동 생성 (기본 24개 V001~V025, `V007` 은 `--seed-admin` opt-in)
...
**설정 파일**:
- [`bootstrap/src/main/resources/application-local.yml`](https://github.com/storkspear/template-spring/blob/main/bootstrap/src/main/resources/application-local.yml) — local Postgres 연결 (`jdbc:postgresql://localhost:5433/postgres`, slug 별 schema 는 DataSource 의 `currentSchema` URL 로 위임)
