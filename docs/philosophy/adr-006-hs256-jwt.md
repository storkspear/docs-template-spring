...
**Status**: Accepted. `common-security/jwt/JwtService.java` 에서 HS256 로 발급과 검증을 처리해요. jjwt 0.13.0 을 사용하고, 서명 키는 환경변수 `JWT_SECRET` 으로 주입됩니다. 로테이션 전략은 `docs/production/setup/key-rotation.md` 에 기록되어 있어요.
...
JWT 표준 기본 알고리즘. 하나의 비밀 키로 서명과 검증 모두.
...
5. **기타 예외** — 위변조·malformed 등 `JwtException` 처리 (`ACCESS_TOKEN_INVALID`)
...
**교훈**: 보안 설정의 실수는 **최대한 이른 시점** 에서 명확한 메시지로 터져야 해요. 부팅 직후 > 첫 토큰 발급 시점 > 첫 검증 시점 > 프로덕션 후 발견 순서로 좋아요.
...
이 `dev-secret-...` 은 **명백히 프로덕션 키가 아님을 이름으로 표명** 해요. 실수로 prod 로 새어나가도 즉시 식별이 가능해요. prod·dev (Mac mini) yml 에는 `${JWT_SECRET}` 만 두고 기본값이 없어요 — 환경변수 누락은 반드시 실패해야 해요.
