...
모든 경로는 **세 가지 prefix** 로 분류:

| Prefix | 용도 |
|---|---|
| `/api/apps/{appSlug}/*` | 앱별 엔드포인트 — auth · device · user 등 ([`ADR-013`](./adr-013-per-app-auth-endpoints.md)) |
| `/api/admin/*` | 운영 콘솔 (cross-app) — RBAC 4티어 ([`ADR-039`](./adr-039-admin-module.md)) |
| `/api/core/*` | 크로스 앱 공통 예약 — 현재 사용 엔드포인트 없음 (코드 0건) |
...
**YAGNI 증명 (부재 확인)**:
- 자체 API 경로 기준 `/v1/` 버전 접두사 — 0건 (`ApiEndpoints.java` 카탈로그 전체에 없음. 외부 API 호출·테스트의 `/v1/nid/me` 같은 Naver·Kakao 엔드포인트 경로는 우리 API 가 아니므로 제외)
