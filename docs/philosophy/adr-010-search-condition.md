...
**Status**: Accepted. `common-web/search/` + `common-persistence/QueryDsl*.java` 에 구현돼 있고, 15 개 연산자 (`eq`, `ne`, `like`, `startsWith`, `endsWith`, `gt`, `gte`, `lt`, `lte`, `in`, `notIn`, `between`, `isNull`, `isNotNull`, `empty`) 와 대소문자 무시 i- 변형 (`ieq`, `ine`, `ilike`, `istartsWith`, `iendsWith`) 을 지원합니다.
...
이 ADR 은 그 인프라의 구체 모양 — Map 키 형식 (`{field}_{operator}`), 연산자 집합 정의, `common-web` 의 순수 DTO 와 `common-persistence` 의 QueryDsl 변환을 두 모듈로 분리한 이유, 그리고 *공장 패턴* 에 맞춰 *컴파일 타임 안전성을 일부 포기한* 트레이드오프 — 를 어떻게 잡았는지 기록합니다.
...
앱의 Controller 는 `PageListRequest` 만 받고, Service 에서 QueryDslPredicateBuilder 를 호출해요. 레이어 간 의존 방향이 깔끔합니다.
...
| `field_like` | 부분 매칭 (대소문자 구분) | `"title_like": "커피"` |
| `field_isNull` | null 여부 | `"deletedAt_isNull": true` |
| `field_isNotNull` | not null 여부 | `"verifiedAt_isNotNull": true` |

> 위 8 개는 초기 결정 시점의 핵심 연산자예요. 현재 구현 (`QueryDslPredicateBuilder`) 은 여기에 `ne`, `startsWith`, `endsWith`, `in`, `notIn`, `between`, `empty` 를 더해 총 15 개를 지원하고, 문자열 비교 계열에는 대소문자를 무시하는 i- 변형 (`ieq`, `ine`, `ilike`, `istartsWith`, `iendsWith`) 도 제공해요. 정확한 목록은 소스의 javadoc 을 참고하세요.
...
- [`QueryDslPredicateBuilder.java`](https://github.com/storkspear/template-spring/blob/main/common/common-persistence/src/main/java/com/factory/common/persistence/QueryDslPredicateBuilder.java) — 443줄, 15개 연산자 (+i 변형)
