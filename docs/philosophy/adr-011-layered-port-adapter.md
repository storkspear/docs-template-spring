...
- **인증 Controller** — 앱 모듈에 없어요. core 공유 `AuthController` ([`ADR-013`](./adr-013-per-app-auth-endpoints.md) B) 가 처리

이 분리는 **엔드포인트 경로** 와 일치해요:
- `/api/apps/<slug>/health`, `/api/apps/<slug>/dashboard` 같은 도메인 경로 → 앱 모듈 `controller/`
- `/api/apps/<slug>/auth/email/signup` 같은 인증 경로 → `core-auth-impl/controller/` 의 공유 컨트롤러
...
```
[공유 AuthController]        — "Port 사용자" (core-auth-impl/controller/, ADR-013 B)
       │
...
[AuthServiceImpl]            — "Primary Adapter" (core-auth-impl/service/)
       ...
       │       │ Spring DI
       │       ▼
       │   [ResendEmailAdapter]  — Secondary Adapter (core-email-impl/)
       │       │ HTTP
       │       ▼
       │   [Resend API]     — 외부 시스템
       │
...
> **갱신 (ADR-013 B, 2026-06)**: 이 교훈의 「스캐폴딩 소스 격하」 결정은 이후 다시 뒤집혔어요. `AuthController` 는 현재 `AuthAutoConfiguration` 이 등록하는 **단일 공유 런타임 빈**이고 `/api/apps/{appSlug}/auth/*` 를 path 변수로 처리합니다 ([`ADR-013`](./adr-013-per-app-auth-endpoints.md) 의 `## 갱신`). 당시 문제였던 `/api/core/auth/*` + ThreadLocal 통합 계정 라우팅과 달리, appSlug 가 URL 에 명시되고 schema 라우팅은 [`ADR-018`](./adr-018-schema-routing-datasource.md) 이 담당해요. 아래는 **역사적 기록**으로 남깁니다.
...
- `AuthController.java` 는 파일은 남지만 **런타임 bean 으로 등록 안 됨** — `new-app.sh` 가 참조할 스캐폴딩 소스로만 존재 *(→ 이후 [`ADR-013`](./adr-013-per-app-auth-endpoints.md) B 로 공유 빈 복귀 — 위 갱신 노트)*
...
- [`EmailPort.java`](https://github.com/storkspear/template-spring/blob/main/core/core-email-api/src/main/java/com/factory/core/email/api/EmailPort.java)
...
**공유 컨트롤러** ([`ADR-013`](./adr-013-per-app-auth-endpoints.md) B):
- [`AuthController.java`](https://github.com/storkspear/template-spring/blob/main/core/core-auth-impl/src/main/java/com/factory/core/auth/impl/controller/AuthController.java) — `AuthAutoConfiguration` 이 등록하는 공유 런타임 빈
