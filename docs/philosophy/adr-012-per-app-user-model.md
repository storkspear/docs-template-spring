...
**Status**: Accepted. *Updated by [ADR-037](./adr-037-core-schema-deprecation.md)* — *core.users* 가 *legacy 잔재* 였다는 부분 정확화 (core schema 자체 폐기 후 각 app schema 의 users 만 남음). 각 앱이 자기 schema 에 독립 users 테이블을 가지는 모델로 운영합니다. `AppSlugVerificationFilter` 가 JWT 의 `appSlug` claim 과 URL 의 `{slug}` 일치를 검증해 런타임 경계를 강제해요. *(갱신: 아래 비교 표·구현 ④ 의 「앱별 `<Slug>AuthController`」 부분은 [ADR-013](./adr-013-per-app-auth-endpoints.md) B 로 **공유 컨트롤러**로 대체됐어요 — per-app USER 모델 자체는 그대로, 컨트롤러 위치만 바뀐 거예요.)*
...
`new-app.sh <slug>` 가 마이그레이션 템플릿 기본 24개 (V001~V025 — `V007` 시드는 `--seed-admin` opt-in) 를 해당 앱 schema 에 생성. 유저·인증 관련 앞부분 발췌:
...
`core-user-impl` 이 갖고 있던 `core/V001__init_users.sql` 템플릿 기준선은 [`ADR-037`](./adr-037-core-schema-deprecation.md) 의 core schema 폐기와 함께 제거됐어요 — 유저 도메인 마이그레이션은 이제 각 앱 schema 경로에만 존재합니다.
...
> **갱신 (ADR-013 B, 2026-06)**: 아래 「앱별 `<Slug>AuthController` / core-auth-impl 은 dead code」 서술은 더 이상 사실이 아니에요. `AuthController` 는 이제 `AuthAutoConfiguration` 이 등록하는 **단일 공유 런타임 빈**이고, `new-app.sh` 는 앱별 복제본을 만들지 않아요 ([`ADR-013`](./adr-013-per-app-auth-endpoints.md) 의 `## 갱신`). 이 ADR 의 본질(**per-app USER 모델** — 앱마다 자기 schema 의 users 테이블 + 단일 `appSlug` claim)은 그대로예요. 컨트롤러 위치만 앱별→공유로 바뀐 것. 아래는 **역사적 기록**으로 남겨요.
...
**원칙**: JWT claim 설계는 첫 발급 시점에 확정해요. "나중에 이름 바꾸지 뭐" 는 프로덕션 직전 큰 비용으로 돌아옴. Breaking change 는 Deprecation 프로세스 ([`ADR-015`](./adr-015-conventional-commits-semver.md)) 경유가 기본이지만, JWT 는 Deprecation 으로도 감당 어려움.
...
- [`JwtService.java`](https://github.com/storkspear/template-spring/blob/main/common/common-security/src/main/java/com/factory/common/security/jwt/JwtService.java) — `issueAccessToken()` (발급) · `validateAccessToken()` (검증)
...
**앱별 users 테이블 마이그레이션**:
- [`tools/new-app/new-app.sh`](https://github.com/storkspear/template-spring/blob/main/tools/new-app/new-app.sh) — 마이그레이션 템플릿 생성 (기본 24개 V001~V025, `V007` 은 `--seed-admin` opt-in)

**부재 확인 (통합 계정 잔재 없음 — ADR-018 이후 기준)**:
- `user_app_access` 조인 테이블 — 0건
- JWT 배열 claim (`"apps": [...]`) 흔적 — 0건 (단일 `appSlug` claim 만)
- *통합 계정용* ThreadLocal 라우팅 — 없음. 현재 레포의 `AbstractRoutingDataSource` (`SchemaRoutingDataSource`) 와 ThreadLocal (`SlugContext`) 은 [`ADR-018`](./adr-018-schema-routing-datasource.md) 의 *slug schema 라우팅* 용으로, 본 ADR 이 기각한 통합 계정 라우팅과는 다른 layer 예요 (위 「이후 갱신 (ADR-018)」 노트 참조)
