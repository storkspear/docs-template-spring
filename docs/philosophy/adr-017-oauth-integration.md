...
> **테이블 리네임 (2026-07-01)**: 본 ADR 에서 `social_identities` 로 지칭하는 테이블은 현재 `auth_social_identities` (엔티티 `AuthSocialIdentity`) 로 리네임됐어요. 아래 본문은 결정 당시 이름을 보존하니, 현재 스키마는 [`data-model`](../reference/data-model.md) 을 참고하세요.
...
- `template-flutter` 의 소셜 로그인 클라이언트 구현 (auth kit — SDK 통합 + 위젯) — 본 ADR 의 짝 프론트 영역이에요. 전용 ADR 문서는 아직 없습니다.
...
- [`core-auth-impl/controller/AuthController.java`](https://github.com/storkspear/template-spring/blob/main/core/core-auth-impl/src/main/java/com/factory/core/auth/impl/controller/AuthController.java) — 공유 런타임 빈 (`{appSlug}` path, ADR-013 B)
...
**유저 식별**
- [`core-user-impl/entity/AuthSocialIdentity.java`](https://github.com/storkspear/template-spring/blob/main/core/core-user-impl/src/main/java/com/factory/core/user/impl/entity/AuthSocialIdentity.java) — `(provider, provider_id)` PK (현 이름: `AuthSocialIdentity` / `auth_social_identities`)
