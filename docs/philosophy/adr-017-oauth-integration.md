...
한 백엔드 인스턴스가 N개 앱을 서비스 (ADR-005, ADR-012). 같은 Google id token 이라도 어느 앱에서 발급됐는지 audience (Google client id) 검증이 필요. Kakao 도 같은 access token 이라도 `app_id` 가 우리 앱과 일치해야 함. 즉 **앱별 credential 매핑이 검증 로직의 핵심**.
...
Google / Apple / Kakao 모두 자체 이메일 검증. 우리 시스템은 신규 소셜 유저를 `email_verified=true` 로 생성. 이메일 인증 메일 발송 안 함.
...
- **외부 의존성 0**: HttpClient (java.net.http) + JJWT 0.13.0 만 사용. spring-security-oauth2-client 도입 안 함.
...
처음엔 `OAuthProvider` 인터페이스 + 구현체 3개로 생각했지만, provider 별 토큰 형태 (JWT vs opaque), 검증 endpoint, audience 필드명, 응답 구조가 모두 달라서 추상화 시그니처가 점점 일반적 (Map) 또는 추상 누수 (`if/else`) 로 가는 걸 발견. **"같은 패턴이지만 코드는 복사"** 가 솔로 친화에 더 부합.
