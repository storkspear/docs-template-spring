...
**Status**: Accepted. 2026-05-01 기준 모든 슬러그별 데이터 격리가 `SchemaRoutingDataSource` 의 ThreadLocal 분기로 동작. ADR-013 (앱별 controller + AuthPort 위임) 의 채택 정신을 service-layer 까지 끝까지 완성.

> **유형**: ADR · **독자**: Level 3 · **읽는 시간**: ~4분

## 결론부터

각 요청에서 URL 의 `{appSlug}` 를 `SlugContext` (ThreadLocal) 에 set → `SchemaRoutingDataSource` (Spring 의 `AbstractRoutingDataSource` 확장) 가 connection 잡을 때 해당 슬러그의 `<slug>DataSource` (HikariCP pool) 로 분기. service Bean 은 단일 (`AuthServiceImpl`), `@Transactional` 도 단일 TM, 다만 connection 자체가 슬러그별이라 INSERT/SELECT 가 자동으로 슬러그 schema 로 라우팅됨.

## 왜 이런 결정이 필요했나?

ADR-013 이 결정한 "앱별 controller + 공통 AuthPort 위임" 패턴이 **service-layer 에서 미완성** 이었음. 도그푸딩 e2e 검증으로 발견:
...
Entity (`@Table(name = "users")`) 가 schema 를 박지 않으므로 connection 의 search_path (DataSource URL 의 `currentSchema=<slug>`) 가 결정. `hibernate.default_schema` 가 박혀 있으면 connection schema 무시되므로 제거.

## ADR-013 거부 사유 vs 현재 상황

ADR-013 이 ThreadLocal 라우팅 거부한 두 이유:

1. **"URL 에서 어느 앱인지 안 보임"** — ADR-012 의 URL 명시 이후 무효. `/api/apps/{slug}/auth/...` 경로에서 slug 가 명시적.
2. **"ThreadLocal + AbstractRoutingDataSource 불안정성"** — Spring 표준 패턴이고 간단하다는 게 사실. ADR-013 이 단순 거부했지만 실제 구현 시 안전 (filter 가 set/clear 보장).

→ ADR-013 의 거부는 그 시점의 결정. ADR-012 가 URL 을 바꿨으니 재검토. 본 ADR-018 이 그 재검토 결과.
