...
- ~~**Pub/Sub bearer token 검증 filter**~~ — **구현됨.** [`ADR-032`](./adr-032-google-webhook-auth.md) 의 `GoogleWebhookAuthFilter` (Bearer JWT 검증, 옵션화)
...
- `core/core-billing-impl/.../controller/IapController.java` — 공유 컨트롤러의 apple/google webhook endpoints (ADR-013 B — 앱별 heredoc 미생성)
