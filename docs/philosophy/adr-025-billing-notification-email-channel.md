...
가장 흔한 케이스는 *앱이 휴대폰에 설치되어 있지 않은 상태* 예요. 사용자가 *결제만 하고 한동안 앱을 사용하지 않거나*, *기기를 바꿔서 앱을 새로 설치하지 않은 상태* 에서 갱신 실패가 발생하면 push 로는 그 사실이 도달할 길이 없습니다. 결제 갱신이나 환불 같은 사건은 *앱을 매일 쓰지 않는 사용자에게도* 도달해야 하는 정보라 push 만으로는 부족해요.

두 번째는 *알림 권한 거부* 입니다. iOS 와 Android 모두 사용자가 *처음 앱 실행 시* 알림 권한을 명시적으로 허용해야 push 가 도달해요. 사용자가 권한을 거부하거나 *나중에 시스템 설정에서 끈* 경우 push 는 발송되어도 사용자에게 보이지 않습니다. 결제 같은 *권한 변경 통지* 가 이런 사용자에게 도달하지 못하면 운영 부담이 직접적으로 늘어요.
