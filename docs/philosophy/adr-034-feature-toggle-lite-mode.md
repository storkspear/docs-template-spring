...
> **갱신 (2026-07-15)**: 이 설계는 `tools/feature.sh` 로 구현 완료됐어요 — `<repo> feature list/enable/disable` 이 실제로 동작합니다.
...
> **갱신 (2026-07-15)**: 아래 4단계는 모두 구현 완료됐어요 — `*AutoConfiguration` conditional · `tools/feature.sh` · `.env.example` 의 `APP_FEATURES_*` 섹션 · `api-smoke-test.sh` 의 feature SKIP.
