...
> 📌 **적용 범위**: 8 도메인 모두 안전 토글 가능. non-leaf 모듈은 의존 측에서 ObjectProvider 로 lazy 의존, leaf 모듈은 단순 ConditionalOnProperty.
...
- `bootstrap/src/test/java/.../FeatureToggleTest` — 8 도메인 동시 비활성 + 부팅 OK + 5 Port bean 미등록 (Testcontainers + @SpringBootTest, 8 test)
- 패턴 분류:
