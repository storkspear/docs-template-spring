...
- [ ] [ADR-037 후속] `BucketProvisioner` 의 *@EnableAsync + @Async* 정식 적용 — 현재 `Thread.startVirtualThread()` 직접 spawn 은 *간이 해결*. Spring 의 `@Async + Executor bean` 정식 패턴 적용 권장 (ApplicationReadyEvent listener block 시 readiness OUT_OF_SERVICE 회피) (2026-05-26)
