...
- [ ] [ADR-037 후속] `application-dev.yml` 의 `management.endpoint.health.show-details: always` 임시 → `never` 복귀 — 본 cycle 진단용으로 추가. dev 안정화 후 base `application.yml` 의 `never` 로 복귀 권장 (2026-05-26)
- [ ] [ADR-037 후속] `StorageAutoConfiguration` 의 `@ConditionalOnProperty(name = "app.storage.minio.endpoint")` empty value 처리 — 빈 값 (`APP_STORAGE_MINIO_ENDPOINT=`) 도 *property exists* 로 판단 → MinioClient bean 생성 시도 → `endpoint must be a non-empty string` 부팅 fail. `@ConditionalOnExpression("'${app.storage.minio.endpoint:}' != ''")` 로 교체 (2026-05-26)
