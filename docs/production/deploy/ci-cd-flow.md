...
> 💡 **push 전 검증** — `<repo> ci-test` 명령이 8단계(Spotless · Build · Docs contract · Docs-check unit · Contract snapshot · users 스키마 드리프트 가드 · 공유 인프라 키 소유권 가드 · gitleaks)를 로컬에서 돌려, push 전에 사전 통과를 보장해요. 이 중 users 스키마 드리프트 가드와 공유 인프라 키 소유권 가드는 대응 워크플로가 없는 로컬 전용 가드예요. 자세한 흐름은 [`CLI 가이드 §4 CI 검증`](../../start/cli-guide.md) 을 참조하세요.
