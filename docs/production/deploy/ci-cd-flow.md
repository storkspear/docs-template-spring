...
`git commit` 부터 사용자에게 노출되기까지 약 15~20분, billed 8분이에요.
...
여기가 이전 설명과 가장 크게 달라진 자리예요. `ci.yml` 과 `docs-check.yml` 은 `on: push` 트리거를 `main` · `develop` 으로만 한정해 둬서, **feature 브랜치 직접 push 에는 어떤 워크플로우도 돌지 않아요**. Actions 분 소비를 아끼려는 의도이고, 코드 검증은 PR 을 열 때 한 번에 모입니다.
...
PR 을 열면 6개 워크플로우가 동시에 돕니다. 코드 게이트(ci · docs-check · security-scan)와 메타 게이트(commit-lint · pr-title · changelog-check)로 나눠 볼 수 있어요.
...
| PR 생성 | 6개 workflow | branch protection 시 머지 차단 |
| Merge | (사용자 판단) | 클릭 안 함 |
