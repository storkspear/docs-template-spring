...
물리·운영 인프라 선택의 결정과 근거, 그리고 어떤 지표가 넘으면 다시 볼지를 추적해요. 코드 설계 결정은 [`Repository Philosophy`](../../philosophy/README.md) 의 38개 ADR 이 따로 담당하고, 이 문서는 DB·스토리지·운영 호스트·엣지·배포 파이프라인처럼 *물리적으로 돈과 전기가 드는* 결정만 모읍니다.
...
| 문서 | 범위 |
|---|---|
| [`Repository Philosophy — 책 안내`](../../philosophy/README.md) | 코드 설계 결정 38개 ADR — 모듈 구조, 포트·어댑터, Mapper 금지, 테스트 전략 |
...
- **결정일**: 2026-04-18
- **결정**: 운영 단일 호스트로 Apple Silicon 맥미니 16GB 를 가정 내에 설치해 사용합니다.
- **근거**:
  - 전기세가 월 4달러 수준이에요. 클라우드 VM 의 월 20달러 이상과 대비됩니다
  - M4 16GB 성능이 AWS t4g.xlarge 급이에요
...
- **Trade-off**:
  ...
  - 디스크 절약을 위해 Loki retention 은 14일, Prometheus retention 은 7일로 잡았어요
- **재검토 트리거**:
  - 맥미니 메모리 12GB 상시 사용 초과 — 관측성 분리 (NAS)
  - 로그 retention 30일 초과 요구 — 단일 인스턴스 Loki 부담
...
- **관련 문서**:
  ...
  - `tools/new-app/new-app.sh` Step 13.5 — `<Slug>DataSourceConfig` 자동 생성
...
- **근거**:
  ...
  - 수동 rollback 경로 — `workflow_dispatch.inputs.version` 으로 과거 SHA 를 재배포해요 (해당 이미지가 GHCR 에 살아있어야 합니다)
- **대안**:
...
| Loki retention 30일 초과 요구 | I-06 관측성 | 관리형 또는 클러스터링 |
...
- [`Repository Philosophy — 책 안내`](../../philosophy/README.md) — 코드 설계 결정 38개 ADR
