...
| **V007** | admin user 시드 (`V007__seed_admin_user.sql`) | `--seed-admin` opt-in — 기본은 미생성, 비밀번호는 랜덤 발급 |
...
다만 이 레포에는 Flyway Gradle 플러그인이 적용돼 있지 않아 `flywayRepair` 태스크가 없어요. repair 에 해당하는 정리는 아래 두 가지 방법으로 수행합니다.

**방법 1 — schema_history SQL 직접 정리 (전 환경 공통)**

repair 가 하는 일을 SQL 로 그대로 수행해요.

```sql
-- 실패한 마이그레이션 entry 제거 (repair 의 핵심 동작)
DELETE FROM <schema>.flyway_schema_history WHERE success = false;

-- checksum 정정 — 부팅 로그의 expected 값을 그대로 반영 (§5.3 참조)
UPDATE <schema>.flyway_schema_history
SET checksum = <expected> WHERE version = '<N>';
```

**방법 2 — schema 리셋 후 AUTO 재기동 (local / test 전용)**

local 은 부팅 시 AUTO migrate 라, schema 를 리셋하고 재부팅하면 history 가 V001 부터 깨끗하게 재구성됩니다.

```bash
psql "$DB_URL" -c "DROP SCHEMA <slug> CASCADE; CREATE SCHEMA <slug>;"   # 데이터 소실 — local 전용
./gradlew :bootstrap:bootRun
```

dev / prod 에서는 방법 1 이 유일한 경로예요. checksum 을 손으로 갱신하는 건 V스크립트 변경을 silent 하게 받아들이는 셈이라, 반드시 [§5.3](#5-3-부팅-시-validate-실패-대응) 의 시나리오 판별을 먼저 거치세요.
...
#         26        |   26    |   add foo   |    t    |  2026-05-02 15:30:21+00

# 5. main push → CI 성공 → deploy.yml 이 workflow_run 으로 자동 트리거
git push origin main
#    (특정 SHA 재배포는 Actions 탭 deploy workflow 의 Run workflow — workflow_dispatch)

# 6. Spring Boot 부팅 (prod profile, validate-only)
#    schema_history 의 V026 와 classpath 의 V026__add_foo.sql checksum 비교
