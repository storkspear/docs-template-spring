...
| 알림 (Discord webhook) | `provisioned` | Alertmanager 컨테이너와 Discord receiver 구성 완료. `DISCORD_WEBHOOK_URL` env 로 즉시 동작. 발사 가능한 알림 룰 5개 (`infra/prometheus/rules.yml` — 에러율·지연·rate-limit·백엔드 다운·MinIO 감시 공백 고지). MinIO 다운/디스크 3단계 규칙은 minio scrape job 이 없어 평가 불가라 주석으로 대기 중 |
