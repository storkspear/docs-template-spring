...
| 마이그레이션 | [Flyway](../../reference/glossary.md#데이터베이스) | 앱 schema 당 V001~V017 | `@Bean(initMethod="migrate")` + advisory lock |
...
[ADR-037](../../philosophy/adr-037-core-schema-deprecation.md) 이후 별도의 `core` schema 는 존재하지 않아요. 각 앱의 `users`·`auth`·`device` 테이블은 그 앱의 schema 안에 V001~V017 로 생성돼요. `core/core-*-impl` 의 Java 코드는 라이브러리 역할로 남아서, 각 앱 DataSource 의 `entityPackagesToScan()` 에 포함돼 앱 schema 의 같은 테이블 위에서 동작해요.
...
> 참고 — 이 cleanup 이후 [`ADR-037`](../../philosophy/adr-037-core-schema-deprecation.md) 로 `core` schema 개념 자체가 폐기됐어요. 그래서 이제는 cleanup 으로 비우는 게 아니라, 처음부터 각 앱 schema 에 V001~V017 이 생성돼요 ([§15.3](#153-현재-schema-상태) 참고).
...
- [`Repository Philosophy — 책 안내`](../../philosophy/README.md) — 38 ADR 인덱스. 특히 [`ADR-007`](../../philosophy/adr-007-solo-friendly-operations.md)
