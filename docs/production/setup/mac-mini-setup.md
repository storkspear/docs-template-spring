...
| 마이그레이션 | [Flyway](../../reference/glossary.md#데이터베이스) | 앱 schema 당 V001~V015 | `@Bean(initMethod="migrate")` + advisory lock |
...
> 참고 — 이 cleanup 이후 [`ADR-037`](../../philosophy/adr-037-core-schema-deprecation.md) 로 `core` schema 개념 자체가 폐기됐어요. 그래서 이제는 cleanup 으로 비우는 게 아니라, 처음부터 각 앱 schema 에 V001~V015 가 생성돼요 ([§15.3](#153-현재-schema-상태) 참고).
