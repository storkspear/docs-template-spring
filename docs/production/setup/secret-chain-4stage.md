...
`.env.prod.example` 과 `.kamal/secrets.example` 의 일치는 수동 grep 으로 확인해요.

```bash
diff <(grep -E '^[A-Z_]+=' .kamal/secrets.example | sort) \
     <(grep -E '^[A-Z_][A-Z0-9_]*=' .env.prod.example | sort)
# 차이가 없어야 정상. .kamal/secrets.example 는 KAMAL_REGISTRY_PASSWORD 한 개만 더 가집니다.
```
...
- [`dogfood-setup §5`](../../start/dogfood-setup.md) — `.env.prod` REQUIRED 5 + OPTIONAL 기능
