...
이 문서는 **어떻게 문서를 쓸 것인가** 의 규칙을 모읍니다. 추상 원칙이 아니라 **`philosophy/` 의 17 개 ADR 파일럿에서 귀납적으로 추출된 규칙** 입니다. 각 규칙마다 구체 샘플 링크가 있어야 하며, 샘플 없는 규칙은 근거가 약합니다.
...
`philosophy/` 의 17 ADR 은 아래 **8 개 필수 섹션** + 선택 섹션으로 구성. 새 설계 결정을 ADR 로 기록할 때 그대로 따름.
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-04-24 | 초기 작성 | `philosophy/` 17 ADR 파일럿에서 귀납 추출 |
