...
| Auth | `auth_email_verification_tokens` | 가입 後 이메일 인증 토큰 (링크) | V004 (+V026 attempts) |
...
### `auth_email_verification_tokens` (V004, V026)

가입 **後** 이메일 인증(링크 클릭 방식) 토큰입니다. `auth_email_verification_codes`(가입 전 코드)와 단계를 구분하려고 별도 테이블로 둡니다.

- **주요 컬럼**: `token_hash` (SHA-256 hex) · `created_at` · `expires_at` · `used_at` · `attempts` (오답 횟수, V026 — 5회 도달 시 토큰 무효).
- **FK**: `user_id` → `users(id)`.
...
앱들이 공유하는 **공개 게시판** 게시물 spine 입니다. 앱별 게시판은 `board` 키로 구분하고, 앱마다 다른 추가 필드는 `properties`(JSONB)에 담아요. **공유(공개) 전용** — 프라이빗 기록은 이 테이블에 오지 않습니다(각 앱 자체 테이블). 모더레이션은 파일 검역과 동일한 상태 전이·soft-delete·purge 패턴입니다: `hide`/`softDelete` 는 행을 유지한 채 상태만 바꾸고, `DELETED` + `purge_at` 경과분은 `ContentPort#purgeExpired` 로 실제 삭제 가능해요 — 다만 형제 도메인 audit/analytics 의 `RetentionScheduler` 와 달리 현재 content 에는 자동 purge 스케줄러가 연동돼 있지 않습니다(수동/외부 호출 시에만 삭제).

- **주요 컬럼**: `slug` · `author_user_id` · `author_type` (`USER`/`ADMIN`) · `authored_by` (ADMIN 작성자 email) · `author_nickname` (작성 시점 닉 스냅샷) · `board` · `title` · `body` · `properties` (JSONB, 앱별 추가필드) · `status` (`ACTIVE`/`HIDDEN`/`DELETED`) · `hidden_at`/`hidden_reason`/`hidden_by` · `deleted_at`/`delete_reason`/`deleted_by`/`purge_at`.
