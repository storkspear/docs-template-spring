...
**해결** — 전제: NAS 자동 백업은 **아직 가동 전**이에요. `infra/scripts/backup-to-nas.sh.example` 은 placeholder 스크립트이고 cron 미등록 상태로, 자동화는 **아직 미구현**이에요. 현재 실제로 쓸 수 있는 건 수동 `pg_dump` 절차(`docs/production/deploy/runbook.md` 의 "백업 — 현재 수동")로, 마지막 수동 백업 시점까지의 데이터만 복구할 수 있어요. 복원은 그 덤프를 새 Postgres(Oracle Cloud Free, 로컬 Docker 등)에 `pg_restore` 한 뒤 `.env` 의 DB URL 을 바꾸면 됩니다. 자동화 전까지는 Supabase 자체 일일 백업이 사실상 유일한 정기 백업이므로, 계정 리스크를 줄이려면 수동 백업 주기를 정해 두거나 백업 자동화를 앞당기고, Supabase Pro 승격으로 계정 안정성과 지원 수준을 끌어올리는 방안이 있어요.
...
**시나리오** — `new-app.sh` 가 공통 마이그레이션 V001~V027 을 앱마다 생성해요(인증·결제·구독·감사·2FA·알림·점유인증·첨부·게시물·애널리틱스 테이블 + opt-in V007 admin 시드). 앱이 20 개면 같은 구조의 테이블이 20 벌 생깁니다.

**영향** — Flyway 실행 시간이 약간 늘고 DB 저장 공간을 조금 더 쓰지만, 기능에는 영향이 없어요.

**해결** — 그대로 수용합니다. 각 앱의 유저 수가 적고 테이블 20 벌의 오버헤드는 무시할 만한 수준이라, 멀티테넌시 격리의 대가로 받아들이는 게 합리적이에요. 도메인 테이블은 V001~V027 이 이미 차 있어 그다음 빈 번호인 V028 부터 직접 작성합니다.
