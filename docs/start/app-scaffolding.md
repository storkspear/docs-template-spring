...
이 문서는 새 앱 도메인 모듈을 만드는 `tools/app/new-app.sh` 와, 그 정확한 역방향인 `tools/app/remove-app.sh` 를 자세히 다뤄요. [`onboarding.md`](./onboarding.md) 의 §3 이 "첫 앱을 올려서 Spring 을 띄우는" 흐름만 짧게 보여줬다면, 이 문서는 그 한 줄 명령이 안에서 무엇을 만들고 무엇을 검증하는지를 끝까지 풀어 줘요.
...
> **한 줄 요약** — 모듈러 모놀리스에 새 앱 도메인을 추가하는 스크립트예요. Gradle 모듈, Java 컨트롤러, Flyway 마이그레이션, `.env` 변수, Postgres schema·role 을 한 번에 만들고, Flyway 적용까지 검증한 뒤 끝나요. admin 시드는 `--seed-admin` 을 붙였을 때만 만들어요.
>
> **최소 명령**: `<repo> new <slug>` (또는 `./tools/app/new-app.sh <slug>`). DB schema·role 생성은 **기본 동작**이라 별도 플래그가 필요 없어요. DB 작업을 빼고 싶을 때만 `--skip-provision-db` 를 붙여요.
...
`<repo> init` (내부적으로 `tools/init/init-local.sh`) 이 해 주는 일은 다음과 같아요.
...
`<repo> new` 는 `<repo> new app <slug>` 와 같고, 둘 다 `./tools/app/new-app.sh <slug>` 를 호출해요. slug 를 생략하면 입력 prompt 가 떠요.
...
| `--seed-admin` | admin 계정 1명을 시드하는 `V007__seed_admin_user.sql` 을 생성 (opt-in). 비밀번호는 랜덤 생성돼 완료 안내에 1회만 출력돼요 — §5.2 참조 |
...
```bash
# tools/app/new-app.sh — _valid_slug 발췌
[[ "$1" =~ ^[a-z][a-z0-9-]*$ ]]
```
...
```gradle
// tools/app/new-app.sh — build.gradle 템플릿 발췌
dependencies {
...
앱 모듈에 생성되는 컨트롤러는 `<SlugPascal>HealthController` 하나뿐이고, 경로는 패키지 루트의 `<SlugPascal>ApiEndpoints` 상수를 참조해요. 이 카탈로그 클래스가 앱 모듈 안에 닫혀 있어서, 코어의 공용 경로 목록을 건드리지 않고도 앱이 자기 path 만 관리해요.
...
인증·결제·인앱 결제 엔드포인트는 앱 모듈에 생기지 않아요. `core-auth-impl` 의 공유 `AuthController` 가 `/api/apps/{appSlug}/auth/*` 경로로 이메일 가입·로그인, Apple·Google 소셜 로그인, 토큰 refresh, 비밀번호 재설정, 2FA 를 전부 처리하고, 결제·인앱 결제도 core 의 공유 컨트롤러가 같은 패턴으로 담당해요.

> 코어의 `AuthController` 는 공유 런타임 bean 하나로 등록되고, `{appSlug}` path 변수로 모든 앱의 인증 요청을 받아요. path 의 slug 와 토큰의 slug 가 일치하는지는 필터가 강제해요. 이 결정의 근거는 [`ADR-013`](../philosophy/adr-013-per-app-auth-endpoints.md) 에 있어요.
...
### 3.5 Flyway Migration — 공통 V001~V027
...
├── V018__init_attachment_file.sql
├── V019__init_user_read_history.sql
├── V020__init_message_send_history.sql
├── V021__init_audit_logs_archive.sql
├── V022__add_payment_refunded_amount.sql
├── V023__add_payment_refunds.sql
├── V024__add_posts.sql
├── V025__add_analytics.sql
├── V026__add_auth_email_verification_tokens_attempts.sql
└── V027__add_login_lockout_to_users.sql
```

`new-app.sh` 가 깔아 주는 공통 마이그레이션은 V001~V027 (V007 제외 26개) 으로, 모든 앱이 똑같이 받는 인증·결제·알림·운영 기반이에요. 어떤 버전이 무엇을 담는지는 아래와 같아요.

| 버전 | 내용 | 비고 |
|---|---|---|
| **V001 ~ V006** | 인증 기반 (users · auth_social_identities · auth_refresh_tokens · email/password 토큰 · devices) | 모든 앱 공통 |
| **V007** | admin user 시드 (`V007__seed_admin_user.sql`) | `--seed-admin` 을 붙였을 때만 생성되는 opt-in 자리. §5.2 |
...
여기서 V007 만 위 디렉토리 목록에 없는 걸 눈치챘을 거예요. `V007__seed_admin_user.sql` 은 `--seed-admin` 을 붙였을 때만, DB provisioning 이 끝난 뒤 별도 단계에서 생성돼요 (§5.2). 본인 도메인 테이블은 그다음 빈 번호(현재 **V028**)부터 직접 작성하면 돼요 (§7).
...
```bash
# tools/app/new-app.sh — inject_env_line 헬퍼 발췌
...
### 5.2 admin user 시드(opt-in)와 Flyway 적용·검증
...
schema·role 생성 (Step 14)
   ↓
V007__seed_admin_user.sql 생성 (Step 15)   ← --seed-admin 을 붙였을 때만
   ↓
Flyway migrate-only 로 공통 마이그레이션 적용 (Step 16)
   ↓
users 테이블 SELECT 검증 (Step 17)          ← --seed-admin 시엔 시드 row 존재까지 확인
```
...
Step 16 은 web server 를 띄우지 않는 migrate-only 모드로 Flyway 를 돌려 공통 마이그레이션(V001~V027)을 적용하고, Step 17 은 `flyway_schema_history` 와 `users` 테이블을 직접 조회해 마이그레이션이 실제로 반영됐는지 확인해요. 그래서 `new` 가 정상 종료했다면 DB 는 이미 준비된 상태예요.
...
```bash
# tools/app/new-app.sh — provision_db 발췌
provision_db() {
...
자동 수행됨:
  ...
  ✅ Flyway 공통 마이그레이션 적용 (Step 16, migrate-only)
  ✅ users 테이블 SELECT 검증 (Step 17)

admin 계정 시드 없음 — 필요하면 --seed-admin 으로 재실행하거나 앱에서 회원가입 하세요.
...
3. 도메인 테이블 작성 (다음 빈 번호부터 — 현재 V028+):
   apps/app-<slug>/src/main/resources/db/migration/<slugPackage>/V028__init_<your-domain>.sql
...
### 7.2 도메인 테이블은 다음 빈 번호부터 (현재 V028)

본인 비즈니스 로직 테이블은 그다음 빈 번호 — 현재 `V028__init_<your-domain>.sql` — 부터 작성해요. V001~V027 이 이미 차 있고, V007 은 도메인이 아니라 `--seed-admin` 전용 관리자 시드 자리예요.
...
공통 마이그레이션(V001~V027)의 체크섬이 맞지 않으면 Flyway 가 거부해요. 공통 마이그레이션을 수정하지 않았다면 원인은 대개 DB 에 남은 이전 실행 흔적이에요. 로컬에서만 schema 를 drop 하고 재생성하세요. 운영에서는 이 방법을 쓰면 안 되고 `flyway repair` 또는 새 번호로 해결해요.
...
| **생성되는 것** | Gradle 모듈 · HealthController 1개(auth·payment·iap 는 core 공유) · ApiEndpoints · AutoConfiguration · DataSource · Flyway V001~V027 (V007 제외) · README · settings.gradle / bootstrap.gradle 업데이트 |
...
| **남은 수동 작업** | (선택) credentials · V028+ 도메인 테이블 · 커밋 |

---

## 10. 앱 모듈 제거 (`remove-app.sh`)

앞의 §1~§9 가 앱을 추가하는 흐름이라면, `tools/app/remove-app.sh` 는 그 정확한 역방향이에요. `new-app.sh` 가 만든 것을 전부 되돌려 앱을 코드 레벨까지 완전히 은퇴시켜요.
