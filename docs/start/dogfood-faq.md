...
[템플릿](../reference/glossary.md#이-레포-고유-용어)의 `tools/init/init-prod.sh` · `init-local.sh` 같은 자동화 코드는 그대로 복사돼요. 하지만 GitHub 설정(Variables · Secrets), Mac mini 의 SSH 키, [GHCR](../reference/glossary.md#운영--인프라) 패키지는 [파생 레포](../reference/glossary.md#이-레포-고유-용어)가 직접 셋업해야 해요. 자동화가 복사된다고 해서 외부 자격까지 따라오는 건 아니에요.
...
# 또는 개별로: bash tools/init/init-local.sh <owner>/<repo> --reinit
#              bash tools/init/init-prod.sh  <owner>/<repo> --reinit
```
...
이 동작을 강제로 막고 싶으면 `--reinit` 없이 `bash tools/init/init-prod.sh`(REPO 인자 없이)를 시도해요. 인자 누락으로 사용법이 출력되면서, 의도치 않은 rename 을 방지할 수 있어요.
...
두 파일은 **파생 레포가 직접 추가**해야 해요(템플릿의 두 파일에 "Phase 2 자동 추가 예정"이라는 주석이 있지만, 현재는 미구현이에요). 새 앱 모듈을 `tools/app/new-app.sh <slug>` 로 추가한 뒤 다음 두 파일을 손봐요.
