...
> 이 절은 도그푸딩에 *반드시 필요한 5 가지* (PAT / Tailscale / SSH / workflow 권한 / Supabase) 만 다룹니다. 운영에서 사용하는 다른 키들 (Cloudflare / Resend / MinIO / PortOne / IAP / Loki / Discord) 의 발급 목적·절차·`.env.prod` 채울 위치를 한 곳에서 보려면 [`운영 키 발급 통합 가이드`](../production/setup/key-issuance.md) 를 참고하세요. 이 §3 은 그 통합 가이드의 *도그푸딩 핵심 5 항목 발췌* 입니다.
...
#### 슬러그별 DataSource — `<SLUG>_DB_URL` 은 비워두기

`<your-backend> new <slug>` 가 새 앱을 추가하면 슬러그별 DataSource 가 자동으로 등록됩니다. 슬러그별 자격은 별도로 채울 필요가 없습니다. `AbstractAppDataSourceConfig` 의 derive 로직이 core 의 `DB_URL` 에서 `currentSchema=<slug>` 부분만 슬러그로 자동 교체하기 때문입니다 (USER 와 PASSWORD 도 core 의 값을 그대로 재사용합니다).

```bash
# .env.prod 의 슬러그별 자격은 비워둡니다 — 자동 derive 됩니다
GYMLOG_DB_URL=
GYMLOG_DB_USER=
GYMLOG_DB_PASSWORD=
```

슬러그별로 별도의 DB role 을 분리하고 싶은 경우 (per-slug role 정책) 명시적으로 채우면 그 값이 우선합니다. 도그푸딩 단계에서는 core 자격을 재사용하는 흐름으로 시작하고, 운영이 안정된 뒤 분리하는 순서를 권장합니다.

→ 코드: `common/common-persistence/.../AbstractAppDataSourceConfig.java` 의 `deriveSlugUrl`
...
#### ⚠ payment — 결제 미사용이라도 어떤 값이든 채워야 운영 부팅이 가능합니다

`<your-backend> new <slug>` 가 생성하는 슬러그 컨트롤러 (`*PaymentController`) 가 `PaymentPort` 를 필수로 의존합니다. prod profile 의 `PortOneProdConfigGuard` 가 부팅 시 v1 키와 webhook secret 의 비어있지 않음을 검증하기 때문에, 미설정 시 `IllegalStateException` 으로 부팅이 차단됩니다.
...
# 실제 결제 시작 — PortOne 콘솔 발급값으로 교체합니다
```

세 키 모두 비어있지 않게 채우면 부팅은 통과합니다. 결제 호출이 실제로 발생하지 않는 한 더미값으로도 운영이 가능하며 (PortOne 이 401 을 반환하는 시점에 호출자가 알아챕니다), PortOne 콘솔에서 발급받는 정식 절차는 [`운영 키 발급 통합 가이드 §4.7`](../production/setup/key-issuance.md#47-portone-pg-결제-featurepayment) 을 참조하세요.

→ 코드: `core/core-payment-impl/.../PaymentAutoConfiguration.java` 의 `portOneProdConfigGuard`
