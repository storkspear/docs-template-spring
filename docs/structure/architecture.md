...
이 문서는 `template-spring` 의 실제 구조 — 무엇이 어디에 있고, 무슨 역할을 하며, 어떻게 연결되는지 — 를 설명해요. 각 결정을 왜 이렇게 설계했는지는 [`philosophy/`](../philosophy/README.md) 디렉토리의 ADR 38 개에 기록되어 있습니다.
...
| `core/*` | `core/core-*-api/`, `core/core-*-impl/` | 공유 플랫폼 기능 — 인증·유저·이메일·SMS·점유인증·디바이스·푸시·billing·iap·payment·스토리지·감사 12 개 도메인 | 상태 있음, `-api`·`-impl` 쌍 |
...
`core-*` 는 모든 앱이 공유하는 플랫폼 로직 라이브러리예요. 12 개 도메인이 각각 `-api` 와 `-impl` 쌍으로 들어 있고, 템플릿에 포함돼서 파생 레포 생성 시 그대로 상속됩니다.
...
`apps/app-<slug>` 는 각 앱의 고유 도메인 로직과 그 앱의 인증·유저 Controller 를 담아요. 파생 레포에서만 작성되고, 템플릿에는 빈 디렉토리만 존재합니다 ([`ADR-013`](../philosophy/adr-013-per-app-auth-endpoints.md)).
