...
이 문서는 `template-spring` 의 실제 구조 — 무엇이 어디에 있고, 무슨 역할을 하며, 어떻게 연결되는지 — 를 설명해요. 각 결정을 왜 이렇게 설계했는지는 [`philosophy/`](../philosophy/README.md) 디렉토리의 ADR 39 개에 기록되어 있습니다.
...
> **ADR 첫 독서 안내** — 결정 근거인 ADR 을 읽기 전에 아래 핵심 용어를 알아두면 좋아요. 처음 보는 용어는 용어집으로 바로 이동할 수 있어요.
> - [`Modular Monolith`](../reference/glossary.md#아키텍처-용어) — 한 프로세스 안의 격리 구획이에요. ADR-001 의 핵심이고요.
> - [`Port / Adapter`](../reference/glossary.md#아키텍처-용어) — `-api` 가 인터페이스 계약(Port), `-impl` 이 구현체(Adapter) 예요. ADR-003, ADR-011 참고.
> - [`AbstractRoutingDataSource`](../reference/glossary.md#데이터베이스) — Spring 의 다중 DataSource 라우팅이에요. 멀티테넌트 schema 의 ADR-018 참고.
> - [`Multitenant`](../reference/glossary.md#아키텍처-용어), [`Tenant`](../reference/glossary.md#아키텍처-용어) — 한 DB 안에서 여러 schema 를 격리해요. 본 레포는 schema-per-app 방식이에요.

---

## 전체 구성 요약

모듈러 모놀리스 구조의 Spring Boot 백엔드예요. Gradle 멀티모듈로 내부가 나뉘어 있고, `bootstrap` 모듈이 전체를 조립해서 단일 fat JAR 로 빌드합니다 ([`ADR-001 · 모듈러 모놀리스`](../philosophy/adr-001-modular-monolith.md)).

### 모듈 4 종류

| 종류 | 위치 | 역할 | 상태 |
|---|---|---|---|
| `common/*` | `common/common-*/` | 뼈대 유틸리티 — 로깅·웹·보안·영속성·테스트 5 개 | 상태 없음 |
| `core/*` | `core/core-*-api/`, `core/core-*-impl/` | 공유 플랫폼 기능 — 인증·유저·이메일·SMS·점유인증·디바이스·푸시·billing·iap·payment·스토리지·감사·운영콘솔(admin)·분석·첨부·게시물 16 개 도메인 (31 개 모듈) | 상태 있음, `-api`·`-impl` 쌍 (admin 은 `-impl` 단독) |
| `apps/*` | `apps/app-<slug>/` | 앱별 도메인 로직 | 템플릿엔 비어 있어요. 파생 레포에서 `new-app.sh` 로 추가해요 |
| `bootstrap` | `bootstrap/` | Spring Boot 엔트리 포인트 | 모든 core-impl 과 common 을 조립 |
...
`core-*` 는 모든 앱이 공유하는 플랫폼 로직 라이브러리예요. 16 개 도메인 · 31 개 모듈이 들어 있고 (15 개 도메인은 `-api`·`-impl` 쌍, `admin` 은 `-impl` 단독), 템플릿에 포함돼서 파생 레포 생성 시 그대로 상속됩니다.

| core 도메인 | 담는 기능 | 근거 ADR |
|---|---|---|
| `auth` | 이메일·소셜 로그인, refresh 회전, 2FA TOTP | [`ADR-013`](../philosophy/adr-013-per-app-auth-endpoints.md), [`ADR-030`](../philosophy/adr-030-2fa-totp.md) |
| `user` | 유저 조회·수정, 소셜 신원, soft delete | [`ADR-012`](../philosophy/adr-012-per-app-user-model.md) |
| `email` | Resend 이메일 발송 (인증 메일·비번 재설정) | [`ADR-024`](../philosophy/adr-024-email-domain-extraction.md) |
| `sms` | CoolSMS(SOLAPI) SMS 발송 | — |
| `phone-auth` | 휴대폰 점유인증 (SMS OTP) | [`ADR-013`](../philosophy/adr-013-per-app-auth-endpoints.md), [`ADR-037`](../philosophy/adr-037-core-schema-deprecation.md) |
| `device` | 디바이스 등록 (푸시 토큰) | — |
| `push` | FCM 푸시 발송 | — |
| `billing` | 구독·플랜 정책 | [`ADR-019`](../philosophy/adr-019-billing-iap-payment-separation.md), [`ADR-020`](../philosophy/adr-020-subscription-domain-model.md) |
| `iap` | Apple·Google 인앱결제 채널 + webhook 검증 | [`ADR-022`](../philosophy/adr-022-iap-server-notifications.md) |
| `payment` | PG 채널 (포트원) | [`ADR-019`](../philosophy/adr-019-billing-iap-payment-separation.md) |
| `storage` | MinIO·R2 오브젝트 스토리지 | — |
| `audit` | 감사 로그 (AOP `@Audited`) | [`ADR-028`](../philosophy/adr-028-audit-log-domain.md) |
| `admin` | cross-app 운영 콘솔 (`/api/admin`, RBAC 4티어) | [`ADR-039`](../philosophy/adr-039-admin-module.md), [`ADR-027`](../philosophy/adr-027-admin-role-authorization.md) |
| `attachment` | 첨부파일 (polymorphic, soft-delete) | — |
| `content` | 공유 게시물 + 모더레이션 | — |
| `analytics` | 제품 이벤트 (append-only) | — |

`apps/app-<slug>` 는 각 앱의 고유 도메인 로직을 담아요. 인증·유저 같은 공통 Controller 는 core 의 **공유 런타임 빈** 이 `{appSlug}` path 변수로 처리하므로 앱 모듈에는 없어요. 파생 레포에서만 작성되고, 템플릿에는 빈 디렉토리만 존재합니다 ([`ADR-013`](../philosophy/adr-013-per-app-auth-endpoints.md)).

앱이 core 를 쓰는 방식은 이래요. 인증과 유저 로직은 `AuthPort`·`UserPort` 인터페이스를 주입받아 위임하고, `core-*-impl` 에는 직접 접근하지 못해요. 이 금지는 Gradle convention plugin 과 ArchUnit 양쪽에서 강제합니다. `ApiResponse`·`@CurrentUser`·`QueryDslPredicateBuilder` 같은 `common-*` 유틸리티는 자유롭게 쓸 수 있고, 각 앱 모듈은 자기 schema 전용 DataSource 를 직접 주입받아요 ([`ADR-005`](../philosophy/adr-005-db-schema-isolation.md)).
...
├── commitlint.config.mjs              # 10 type × 23 scope × 72자 제한
...
│       ├── contract-snapshot.yml      # API 계약 스냅샷 검증
...
│   │   ├── dogfood-pitfalls.md        # 15 함정 reference
...
│   ├── philosophy/                    # L3 — 39 개 ADR
│   │   ├── README.md                  # ADR 인덱스 + 테마별 그룹
│   │   └── adr-001 ~ adr-039.md       # Architecture Decision Records
...
│   │       ├── ApiEndpoints.java                  # 경로 상수 (/api/apps/*, /api/admin)
...
│   │       │   ├── PageListRequest.java           # page + size + sorts + filterModel
...
│   │       ├── AuthenticatedUser.java             # { userId, email, appSlug, role, permissions }
...
│   │       │                                      #   BCrypt 해시 JSON 으로 users.totp_backup_codes 에 저장, 1회용)
│   │       ├── entity/
│   │       │   ├── AuthRefreshToken.java
│   │       │   ├── AuthEmailVerificationCode.java # 가입 전 인증 코드 (verify-BEFORE-signup)
...
│   │       └── controller/DeviceController.java   # POST/DELETE /api/apps/{appSlug}/devices (공유 런타임 빈)
│   │
│   ├── core-push-api/                 # 푸시 포트
...
│   │       ├── FcmMessagingFactory.java           # 앱별 FirebaseMessaging 인스턴스 생성
│   │       ├── NoOpPushAdapter.java               # FCM 미설정 환경 fallback
│   │       ├── PushAppCredentialProperties.java   # 앱별 FCM 서비스 계정 자격
...
│   │
│   ├── core-attachment-api/           # 첨부파일 포트
│   ├── core-attachment-impl/          # AttachmentFile (polymorphic, soft-delete) + StoragePort 연동
│   │
│   ├── core-content-api/              # 공유 게시물 포트
│   ├── core-content-impl/             # PostController (/api/apps/{appSlug}/posts) + 모더레이션 상태
│   │
│   ├── core-analytics-api/            # 제품 이벤트 포트
│   ├── core-analytics-impl/           # AnalyticsEvent (append-only)
│   │
...
Gradle 이 잡지 못하는 패턴은 ArchUnit 22 개 규칙 r1~r22 로 빌드 시 검증합니다. 같은 패키지 이름 규칙, Entity 노출 금지, `*Mapper` 클래스 금지 같은 것들이에요 (r12 는 현 구조에서 vacuous 라 유보). 상세는 [`Architecture Rules (ArchUnit)`](./architecture-rules.md) 를 참조하세요.
...
[AppSlugVerificationFilter]             (common-security)
      ...
      ▼
[UserController.getMyProfile(@CurrentUser AuthenticatedUser user)]  (core-user-impl, 공유 런타임 빈)
      │ CurrentUserArgumentResolver 가 AuthenticatedUser 주입
      ▼
UserServiceImpl.getProfile(user.userId())          (core-user-impl, sumtally DataSource 사용)
      │
      ▼
...
RefreshTokenService.rotate()
      │
      ├─→ tokenHash = TokenGenerator.sha256Hex(refreshToken)   # 결정적 해시라 인덱스 조회 가능
...
WithdrawService.withdraw(userId, reason)
      │
      ├─→ UserPort.softDelete(userId)        # User.deleted_at = now
      ├─→ RefreshTokenService.revokeAllForUser(userId)
      │
      └─→ (디바이스 정리·30일 후 hard delete 는 후속 작업 — Phase 1 TODO)
```
...
protected DataSource buildDataSource(String url, String user, String pw) {
    ...
    cfg.setPassword(pw);
    cfg.setMaximumPoolSize(5);          // DEFAULT_POOL_SIZE — 앱별 독립 풀
...
`bootstrap` 의 `RoutingDataSourceConfig` 가 `@Primary` 로 라우팅과 JPA 를 통합하고, 각 앱 모듈의 `<Slug>DataSourceConfig` 가 자기 슬러그 DataSource 를 등록해요. 두 설정이 공존하며, 공통 빌더는 `AbstractAppDataSourceConfig` 예요. ADR-037 이후 core schema 와 coreDataSource Bean 은 폐기됐고, slug 없는 DB 접근은 `IllegalStateException` 으로 막혀요 (fail-secure).
...
### 핵심 레퍼런스
- [`Repository Philosophy — 책 안내`](../philosophy/README.md) — 39 개 ADR 설계 결정의 근거
