...
JWT 기반 stateless 인증 체계는 다섯 개의 보안 필터와 한 개의 토큰 서비스로 구성돼요. 핵심 부품은 SecurityConfig, JwtService, JwtAuthFilter, `@CurrentUser`, appSlug 검증, Refresh Token 회전이에요.
...
| `permissions` | List\<String\> | 운영 콘솔 토큰 전용 `PERM_*` 권한 목록 (앱 유저 토큰엔 없음) |
...
public String issueAccessToken(long userId, String email, String appSlug, String role) {
    return buildAccessToken(
            userId, email, appSlug, role, List.of(), properties.accessTokenTtl());
}

private String buildAccessToken(
        long userId, String email, String appSlug, String role,
        List<String> permissions, Duration ttl) {
    ...
    return builder.issuer(properties.issuer())
...
앱 유저용 `issueAccessToken` 은 `permissions` 없이 발급하고, 운영 콘솔용 `issueAdminAccessToken` 은 RBAC 권한(`PERM_*`) 목록과 콘솔 전용 TTL 로 같은 빌더를 재사용해요.
...
public AuthenticatedUser validateAccessToken(String token) {
    try {
        ...
        return new AuthenticatedUser(userId, email, appSlug, role, permissions);
...
public record AuthenticatedUser(
    ...
    public AuthenticatedUser {
        ...
        // 운영 콘솔 토큰만 RBAC 권한(PERM_*)을 싣는다. 앱 유저 토큰은 빈 목록.
...
다섯 번째 컴포넌트 `permissions` 는 운영 콘솔 JWT 의 `permissions` claim 에서 오는 `PERM_*` 권한 목록이에요. `JwtAuthFilter` 가 `GrantedAuthority` 로 변환하고 `SecurityConfig` 가 `/api/admin/**` 리소스별 `hasAuthority` 검사에 사용해요. 앱 유저 토큰은 항상 빈 목록입니다.
...
| 공개 경로 | 용도 |
|---|---|
| `/email/send-code`, `/email/verify-code` | 가입 전 이메일 인증 (verify-BEFORE-signup — 코드 발송·검증) |
...
`AuthRefreshToken` 엔티티(`.../entity/AuthRefreshToken.java`)의 주요 컬럼이에요.
...
public AuthTokens rotate(String rawRefreshToken, String appSlug) {
    ...
    AuthRefreshToken existing = refreshTokenRepository.findByTokenHash(incomingHash)
    ...
    // 정상 회전 — old 를 used 로 조건부 원자 업데이트 (rotate race 차단)
    int marked = refreshTokenRepository.markUsedIfUnused(existing.getId());
    if (marked == 0) {
        // 동시 회전 or 재사용 공격 — 탈취 감지와 동일하게 처리
        revokeFamily(existing);
        throw new AuthException(AuthError.INVALID_TOKEN);
    }
...
재사용 탐지(reuse detection)가 핵심이에요. 이미 회전에 사용된(`used_at IS NOT NULL`) token 이 또 들어오면 탈취로 판정합니다. 이 경우 같은 `family_id` 의 모든 active token 을 `REQUIRES_NEW` 트랜잭션으로 분리해 무효화해요(`revokeFamily`). 별도 트랜잭션을 쓰는 이유는, 현재 트랜잭션이 exception 으로 rollback 되더라도 revoke 는 독립 커밋으로 유지되어야 하기 때문이에요.
...
| `core-auth-impl/.../entity/AuthRefreshToken.java` | JPA 엔티티 (token_hash / family_id / used_at / revoked_at) |
| `core-auth-impl/.../repository/AuthRefreshTokenRepository.java` | `findByTokenHash`, `markUsedIfUnused`, `revokeAllByFamilyId`, `revokeAllByUserId` |
