...
## 3. Routing DataSource layer (bootstrap)

`bootstrap/.../config/RoutingDataSourceConfig.java`

ADR-037 이후 *core schema / coreDataSource / coreFlyway Bean 은 폐기*. bootstrap 모듈은 *routing + JPA 통합 layer* 만 담당해요 — `SchemaRoutingDataSource` 가 모든 `<slug>DataSource` Bean 을 자동 수집해 `SlugContext` (ThreadLocal) 의 slug 로 connection 을 분기합니다.
...
public class RoutingDataSourceConfig {
    ...
    public DataSource dataSource(Map<String, DataSource> allDataSources) {
        // <slug>DataSource 모두 routing target 등록 (자기 자신 dataSource 제외)
...
`RoutingDataSourceConfig` 의 `@Primary` 가 *모든 core repository* 의 default EMF/TM 로 작동하면서, *각 app DataSource 는 routing target 으로만 등록* 됨 → core repository 가 *현재 `SlugContext` 의 slug schema* 에서 *자동 동작*.
...
### slug 없는 DB 접근 = fail-secure (ADR-037)

`SchemaRoutingDataSource.determineCurrentLookupKey()` 가 `SlugContext.get() == null` 시 `IllegalStateException` 을 던집니다 — 기존 silent `core` fallback 폐기. 비인증 endpoint (actuator/health) 는 DB 접근 없음, 인증 endpoint 는 `AppSlugMdcFilter` 가 JWT 의 `appSlug` claim 또는 `/api/apps/{slug}/...` path 에서 slug 추출 후 set.
...
| `bootstrap/RoutingDataSourceConfig` | `@Primary` routing dataSource + 공유 EMF/TM + 멀티테넌트 dbHealthContributor (ADR-037) |
...
| `bootstrap/.../config/RoutingDataSourceConfig.java` | `@Primary` routing + 공유 EMF/TM + 멀티테넌트 dbHealthContributor (ADR-037) |
